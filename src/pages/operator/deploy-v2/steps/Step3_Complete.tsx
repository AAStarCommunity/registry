...
export function Step3_Complete({ mode, resources, onRestart }: Step3Props) {
  ...
  const ENTRY_POINT = getEntryPoint("sepolia");
  ...
  const registerToSuperPaymaster = async () => {
    ...
    try {
      ...
      console.log("=== Starting SuperPaymaster Registration (One-Step) ===");
      ...
      console.log("✅ GT approved");

      // Step 2: Approve aPNTs
      console.log("Step 2: Approving aPNTs for SuperPaymaster...");
      const approveAPNTsTx = await aPNTs.approve(
        superPaymasterAddress,
        initialAPNTs,
      );
      ...
      // Step 3: One-step registration with auto-stake
      console.log("Step 3: Calling registerOperatorWithAutoStake...");
      console.log(
        `  - GT Stake: ${ethers.formatEther(stakeAmount)} GT`,
      );
      console.log(
        `  - aPNTs Deposit: ${ethers.formatEther(initialAPNTs)} aPNTs`,
      );
      console.log(`  - Supported SBTs: ${supportedSBTs.length}`);
      console.log(`  - xPNTs Token: ${xPNTsToken}`);
      console.log(`  - Treasury: ${treasury}`);

      const registerTx = await superPaymaster.registerOperatorWithAutoStake(
        stakeAmount,
        initialAPNTs,
        supportedSBTs,
        xPNTsToken,
      ...
      console.log("✅ Registration complete! (Auto-staked GT + Deposited aPNTs)");

      console.log("=== One-Step Registration Complete ===");
  ...
  const registerPaymasterToRegistry = async () => {
    ...
    try {
      ...
      if (mode === "aoa") {
        ...
        paymasterAddress = core.superPaymasterV2;
      }
